...
void external_test() {
	...
	// 2.而 Date - int 的重载，会被编译器尝试理解成 不是我们期望的 Date - Date(int) 的形式
	...
	// 
	// 另外我发现这个C2666编译器错误有一定几率降级为E0350并通过编译：错误(活动) E0350 有多个运算符 "-" 与这些操作数匹配
	// 并且这个现象比较玄学，在这里不能跑的，我把文件没改动直接复制到另一个工程里就能跑了。（比如当前这个工程就能跑）
	//									
}
